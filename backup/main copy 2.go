...
func executeTask(task Task) bool {
	...
	if task.Method == "POST" {
		// Prepare the request body for POST method
		if task.User != "" && task.Token != "" {
			// Include user and token in the body
			postData := map[string]string{
				"user":  task.User,
				"token": task.Token,
			}
			postBody, _ := json.Marshal(postData)
			req, err = http.NewRequest("POST", task.URL, bytes.NewBuffer(postBody))
			req.Header.Set("Content-Type", "application/json")
		} else {
			// No authentication, empty POST body
